...
class Purchase:
    ...
    def __str__(self):
        ...
        for i in range(len(self.products)):
            lst = lst + f'\n{list(self.products.keys())[i]}: {list(self.products.values())[i]} pcs.'
        return f'User: {self.user},\nItems:{lst}'

    def get_total(self):
        self.total = 0
        for i in range(len(self.products.keys())):
            self.total += list(self.products.keys())[i].price * list(self.products.values())[i]
        return self.total
